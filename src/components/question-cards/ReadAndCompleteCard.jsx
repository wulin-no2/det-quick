...
const ReadAndCompleteCard = ({
  ...
  const [answers, setAnswers] = useState(
    validBlankList.map((blank) => blank.clues.map((clue) => clue || ""))
  );
  const [backgroundColors, setBackgroundColors] = useState(
    validBlankList.map((blank) => Array(blank.clues.length).fill("white"))
  );
  const [showCorrectAnswer, setShowCorrectAnswer] = useState(false);
  const [buttonText, setButtonText] = useState("Submit");
  const inputRefs = useRef(
    validBlankList.map((blank) => Array(blank.clues.length).fill(null))
  );
  ...
  if (
    !questionDetail ||
    !questionDetail.sentenceTemplate ||
    !questionDetail.blankList
  ) {
    return <div></div>;
  }
  ...
  const getNextEnabledInput = (placeholderIndex, charIndex) => {
    for (let i = placeholderIndex; i < inputRefs.current.length; i++) {
      for (
        let j = i === placeholderIndex ? charIndex : 0;
        j < inputRefs.current[i].length;
        j++
      ) {
  ...
  const getPreviousEnabledInput = (placeholderIndex, charIndex) => {
    for (let i = placeholderIndex; i >= 0; i--) {
      for (
        let j =
          i === placeholderIndex ? charIndex : inputRefs.current[i].length - 1;
        j >= 0;
        j--
      ) {
        if (inputRefs.current[i][j] && !inputRefs.current[i][j].disabled) {
  ...
  const handleInputChange = (placeholderIndex, charIndex, event) => {
    ...
    if (event.target.value) {
      nextFocusRef.current = getNextEnabledInput(
        placeholderIndex,
        charIndex + 1
      );
    }
  ...
  const handleKeyDown = (placeholderIndex, charIndex, event) => {
    if (event.key === "ArrowRight") {
      ...
      nextFocusRef.current = getPreviousEnabledInput(
        placeholderIndex,
        charIndex - 1
      );
      if (nextFocusRef.current) {
    ...
    if (buttonText === "Submit") {
      ...
      const newBackgroundColors = answers.map((answerGroup, i) =>
        answerGroup.map((answer, j) =>
          validBlankList[i].clues[j] !== null
            ? grey[100]
            : answer === validBlankList[i].answer[j]
            ? green[100]
            : red[100]
        )
      );
                  ...
                  validBlankList[index]?.clues?.map((clue, clueIndex) => {
                    ...
                    return (
                      <input
                        ...
                        onChange={(event) =>
                          handleInputChange(index, clueIndex, event)
                        }
                        onKeyDown={(event) =>
                          handleKeyDown(index, clueIndex, event)
                        }
                        style={{
                          width: "24px",
                          height: "28px",
                          ...
                          backgroundColor:
                            validBlankList[index]?.clues?.[clueIndex] !== null
                              ? grey[100]
                              : backgroundColors[index]?.[clueIndex] || "white", // Set background color based on state
                          borderRadius:
                            clueIndex === 0
                              ? "4px 0 0 4px" // First character
                              : clueIndex ===
                                validBlankList[index]?.clues?.length - 1
